 ...
 * @type {number[]}
 */
DND5E.CHARACTER_EXP_LEVELS = [ 0, 240, 480, 720, 960, 1440, 1920, 2400, 2880, 3360, 3840, 4320, 4800, 5280, 5760, 6240, 6720, 7200, 7680, 8160, ];
...
class AdvancementConfig extends FormApplication {
  ...
  get title() {
    ...
    return `${game.i18n.format("DND5E.AdvancementConfigureTitle", {
      item: this.item.name,
    })}: ${type}`;
  }
  ...
  getData() {
    const levels = Object.fromEntries(
      Array.fromRange(CONFIG.DND5E.maxLevel + 1).map((l) => [l, l])
    );
  ...
  async _onItemDelete(event) {
    ...
    const items = foundry.utils.getProperty(
      this.advancement.data.configuration,
  ...
  async _onDrop(event) {
    ...
    if (item.uuid === this.item.uuid) {
      return ui.notifications.error(
        game.i18n.localize("DND5E.AdvancementItemGrantRecursiveWarning")
      );
    }
    ...
    if (existingItems.includes(item.uuid)) {
      return ui.notifications.warn(
        game.i18n.localize("DND5E.AdvancementItemGrantDuplicateWarning")
      );
    }
...
class Advancement {
  ...
  static get defaultData() {
    const data = {
      _id: null,
      type: this.typeName,
      configuration: foundry.utils.deepClone(
        this.metadata.defaults.configuration
      ),
  ...
  updateSource(updates) {
    ...
    if (idx < 0)
      throw new Error(
        `Advancement of ID ${this.id} could not be found to update`
      );
  ...
  _prepareSpellChanges(spell) {
    ...
    if (spell.preparation)
      updates["system.preparation.mode"] = spell.preparation;
    if (spell.uses?.max) {
      updates["system.uses.max"] = spell.uses.max;
      if (Number.isNumeric(spell.uses.max))
        updates["system.uses.value"] = parseInt(spell.uses.max);
      else {
        try {
          const rollData = this.actor.getRollData({ deterministic: true });
          const formula = Roll.replaceFormulaData(spell.uses.max, rollData, {
            missing: 0,
          });
...
class HitPointsAdvancement extends Advancement {
   ...
   */
  total() {
    return Object.keys(this.data.value).reduce(
...
class HitPointsFlow extends AdvancementFlow {
  ...
  _updateObject(event, formData) {
    ...
    throw new Advancement.ERROR(
      game.i18n.localize(`DND5E.AdvancementHitPoints${errorType}Error`)
    );
  }
}
...
class ItemGrantAdvancement extends Advancement {
  ...
  summaryForLevel(level, { configMode = false } = {}) {
    ...
    if (!this.data.value.added || configMode) {
      return this.data.configuration.items.reduce(
        (html, uuid) => html + dnd5e.utils.linkForUuid(uuid),
        ""
      );
    }
  ...
  async apply(level, data, retainedData = {}) {
    const items = [];
    const updates = {};
    const spellChanges = this.data.configuration.spell
      ? this._prepareSpellChanges(this.data.configuration.spell)
      : {};
...
class ItemGrantConfig extends AdvancementConfig {
  ...
  getData() {
    ...
    context.showSpellConfig = context.data.configuration.items
      .map(fromUuidSync)
      .some((i) => i.type === "spell");
    return context;
  }
  ...
  _validateDroppedItem(event, item) {
    ...
    throw new Error(
      game.i18n.format("DND5E.AdvancementItemTypeInvalidWarning", { type })
    );
  }
}
  ...
  async getData() {
    ...
    const added =
      this.retainedData?.items.map((i) =>
        foundry.utils.getProperty(i, "flags.dnd5e.sourceId")
...
class ScaleValueAdvancement extends Advancement {
  ...
  get levels() {
    return Array.from(
      Object.keys(this.data.configuration.scale).map((l) => Number(l))
    );
  }
  ...
  prepareValue(level) {
    ...
    if (this.data.configuration.type === "dice")
      return `${value.n ?? ""}d${value.die}`;
    return `${value.value}`;
  }
   ...
   */
  formatValue(level) {
    if (this.data.configuration.type !== "distance")
      return this.prepareValue(level);
    const value = this.valueForLevel(level);
    if (value == null) return null;
    return `${value.value} ${
      CONFIG.DND5E.movementUnits[this.data.configuration.distance.units]
    }`;
  }
}
...
class ScaleValueConfig extends AdvancementConfig {
  ...
  getData() {
    ...
    data.typeHint = game.i18n.localize(
      `DND5E.AdvancementScaleValueTypeHint${config.type.capitalize()}`
    );
    data.types = Object.fromEntries(
      Object.entries(ScaleValueAdvancement.TYPES).map(([key, label]) => [
        key,
        game.i18n.localize(label),
      ])
    );
    data.faces = Object.fromEntries(
      [2, 3, 4, 6, 8, 10, 12, 20].map((die) => [die, `d${die}`])
    );
  ...
  _prepareLevelData() {
    let lastValue = null;
    return Array.fromRange(CONFIG.DND5E.maxLevel + 1)
      .slice(1)
      .reduce((obj, level) => {
        obj[level] = {
  ...
  _formatPlaceholder(placeholder) {
    if (this.advancement.data.configuration.type === "dice") {
      return {
        n: placeholder?.n ?? "",
        die: placeholder?.die ? `d${placeholder.die}` : "",
      };
    }
  ...
  activateListeners(html) {
    super.activateListeners(html);
    this.form
      .querySelector("input[name='data.title']")
  ...
  _onChangeTitle(event) {
    ...
    this.form.querySelector(
      "input[name='data.configuration.identifier']"
    ).placeholder = slug;
  }
  ...
  async _updateObject(event, formData) {
    ...
    if (
      typeChange &&
      formData["data.configuration.type"] !==
        this.advancement.data.configuration.type
    ) {
...
class AdvancementConfirmationDialog extends Dialog {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template:
        "systems/dnd5e/templates/advancement/advancement-confirmation-dialog.hbs",
      jQuery: false,
    });
  }
...
class AdvancementManager extends Application {
  ...
  get title() {
    ...
    const step =
      visibleIndex < 0
        ? ""
        : game.i18n.format("DND5E.AdvancementManagerSteps", {
  ...
  static forModifyChoices(actor, itemId, level, options) {
    ...
    const currentLevel =
      clonedItem.system.levels ??
      clonedItem.class?.system.levels ??
  ...
  getData() {
    ...
    return {
      ...
      header: item.name,
      subheader: level
        ? game.i18n.format("DND5E.AdvancementLevelHeader", { level })
        : "",
      steps: {
        current: visibleIndex + 1,
        total: visibleSteps.length,
  ...
  async close(options = {}) {
    if (!options.skipConfirmation) {
      return new Dialog({
        title: `${game.i18n.localize("DND5E.AdvancementManagerCloseTitle")}: ${
          this.actor.name
        }`,
        ...
        buttons: {
          close: {
            ...
            label: game.i18n.localize(
              "DND5E.AdvancementManagerCloseButtonStop"
            ),
          ...
          continue: {
            ...
            label: game.i18n.localize(
              "DND5E.AdvancementManagerCloseButtonContinue"
            ),
          },
        },
        default: "close",
  ...
  static async createDialog(item, { rejectClose = false, options = {} } = {}) {
    return new Promise((resolve, reject) => {
      const dialog = new this(
        item,
        {
          title: `${game.i18n.localize("DND5E.AdvancementSelectionTitle")}: ${
            item.name
          }`,
          buttons: {
            submit: {
...
class ActiveEffect5e extends ActiveEffect {
  ...
  apply(actor, change) {
    ...
    if (change.key.startsWith("flags.dnd5e."))
...
async function d20Roll({
  ...
  const { advantageMode, isFF } = _determineAdvantageMode({
    advantage,
    disadvantage,
    fastForward,
    event,
...
class TraitSelector extends DocumentSheet {
  ...
  _prepareUpdateData(formData) {
    ...
    // Obtain choices
    const chosen = Object.entries(formData)
      .filter(([k, v]) => k !== "custom" && v)
      .map(([k]) => k);
...
class ProficiencySelector extends TraitSelector {
  ...
  async getData() {
    ...
    const data = super.getData();
    data.choices = await this.constructor.getChoices(
      this.options.type,
      chosen,
      this.options.sortCategories
    );
    return data;
  }
  ...
  static async getChoices(type, chosen = []) {
    let data = Object.entries(CONFIG.DND5E[`${type}Proficiencies`]).reduce(
      (obj, [key, label]) => {
    ...
    if (type === "tool") {
      data.vehicle.children = Object.entries(CONFIG.DND5E.vehicleTypes).reduce(
        (obj, [key, label]) => {
          obj[key] = { label: label, chosen: chosen.includes(key) };
          return obj;
        },
    ...
    for (const category of Object.values(data)) {
      ...
      category.children = dnd5e.utils.sortObjectEntries(
        category.children,
        "label"
      );
    }
...
class AbilityUseDialog extends Dialog {
  ...
  static async create(item) {
    ...
    const html = await renderTemplate(
      "systems/dnd5e/templates/apps/ability-use.hbs",
      data
    );
    ...
    const label = game.i18n.localize(
      `DND5E.AbilityUse${data.isSpell ? "Cast" : "Use"}`
    );
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const spellLevels = Array.fromRange(10)
      .reduce((arr, i) => {
        ...
        arr.push({
          ...
          label:
            i > 0
              ? game.i18n.format("DND5E.SpellLevelSlot", {
                  level: label,
                  n: slots,
                })
              : label,
    ...
    if (pact.level >= lvl) {
      spellLevels.push({
        level: "pact",
        label: `${game.i18n.format("DND5E.SpellLevelPact", {
          level: pact.level,
          n: pact.value,
        })}`,
        canCast: true,
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if (quantity <= 0)
      return game.i18n.localize("DND5E.AbilityUseUnavailableHint");

    // Abilities which use Recharge
    if (recharge.value) {
      return game.i18n.format(
        recharge.charged
          ? "DND5E.AbilityUseChargedHint"
          : "DND5E.AbilityUseRechargeHint",
        {
          type: game.i18n.localize(`DND5E.ItemType${item.type.capitalize()}`),
        }
      );
    ...
    if (item.type === "consumable") {
      ...
      if (uses.value > 1) str = "DND5E.AbilityUseConsumableChargeHint";
        ...
        str = "DND5E.AbilityUseConsumableDestroyHint";
      else if (item.system.quantity > 1)
        str = "DND5E.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(
          `DND5E.Consumable${item.system.consumableType.capitalize()}`
        ),
        value: uses.value,
...
class Item5e extends Item {
  ...
  get class() {
    ...
    return (this._classLink ??= this.parent.items.find(
      (i) => i.type === "class" && i.system.identifier === cid
    ));
  }
  ...
  get spellcasting() {
    ...
    const classSpellcasting = isSubclass
      ? this.class?.system.spellcasting
      : spellcasting;
    const subclassSpellcasting = isSubclass
      ? spellcasting
      : this.subclass?.system.spellcasting;
    if (subclassSpellcasting && subclassSpellcasting.progression !== "none")
  ...
  _prepareEquipment() {
    this.labels.armor = this.system.armor.value
      ? `${this.system.armor.value} ${game.i18n.localize("DND5E.AC")}`
      : "";
  }
  ...
  _prepareFeat() {
    ...
    if (act?.type === types.legendary)
      this.labels.featType = game.i18n.localize("DND5E.LegendaryActionLabel");
    else if (act?.type === types.lair)
      this.labels.featType = game.i18n.localize("DND5E.LairActionLabel");
    else if (act?.type) {
      this.labels.featType = game.i18n.localize(
        this.system.damage.length ? "DND5E.Attack" : "DND5E.Action"
      );
  ...
  _prepareAdvancement() {
    ...
    this.advancement = {
      ...
      byLevel: Object.fromEntries(
        Array.fromRange(CONFIG.DND5E.maxLevel + 1)
    ...
    for (const advancementData of this.system.advancement ?? []) {
      const Advancement =
        dnd5e.advancement.types[`${advancementData.type}Advancement`];
  ...
  getCriticalThreshold() {
    ...
    return Math.min(
      this.system.critical?.threshold ?? 20,
      actorThreshold ?? 20
    );
  }
  ...
  prepareMaxUses() {
    ...
    if (this.isOwned && !Number.isNumeric(max)) {
      ...
      try {
        ...
        const message = game.i18n.format("DND5E.FormulaMalformedError", {
          property,
          name: this.name,
        });
  ...
  async use(config = {}, options = {}) {
    ...
    const requireSpellSlot =
      isSpell &&
      is.level > 0 &&
      CONFIG.DND5E.spellUpcastModes.includes(is.preparation.mode);
     ...
     */
    if (
      Hooks.call("dnd5e.preItemUsageConsumption", item, config, options) ===
      false
    )
      return;
     ...
     */
    if (
      Hooks.call("dnd5e.itemUsageConsumption", item, config, options, usage) ===
      false
    )
      return;
    ...
    if (config.createMeasuredTemplate) {
      try {
        templates = await dnd5e.canvas.AbilityTemplate.fromItem(
          item
        )?.drawPreview();
      } catch (err) {}
    }
  ...
  async getChatData(htmlOptions = {}) {
    ...
    if (
      ...
      if (data.attunement === CONFIG.DND5E.attunementTypes.REQUIRED) {
        props.push(
          CONFIG.DND5E.attunements[CONFIG.DND5E.attunementTypes.REQUIRED]
        );
      }
      props.push(
        game.i18n.localize(
  ...
  async rollAttack(options = {}) {
    ...
    const elvenAccuracy =
      (flags.elvenAccuracy &&
        CONFIG.DND5E.characterFlags.elvenAccuracy.abilities.includes(
          this.abilityMod
        )) ||
      undefined;
  ...
  async rollDamage({
    ...
    const actionFlavor = game.i18n.localize(
      this.system.actionType === "heal" ? "DND5E.Healing" : "DND5E.DamageRoll"
    );
  ...
  async rollRecharge() {
    ...
    if (rollConfig.chatMessage) {
      const resultMessage = game.i18n.localize(
        `DND5E.ItemRecharge${success ? "Success" : "Failure"}`
      );
      roll.toMessage({
  ...
  async rollToolCheck(options = {}) {
    ...
    const rollConfig = foundry.utils.mergeObject(
      {
        ...
        reliableTalent:
          ...
          this.actor.getFlag("dnd5e", "reliableTalent"),
        messageData: {
          speaker:
     ...
     * @returns {boolean}                    Explicitly return false to prevent the roll from being performed.
     */
    if (Hooks.call("dnd5e.preRollToolCheck", this, rollConfig) === false)
      return;
  ...
  static async _onChatCardAction(event) {
    ...
    if (!item) {
      const err = game.i18n.format("DND5E.ActionWarningNoItem", {
  ...
  async createAdvancement(type, data = {}, { showConfig = true } = {}) {
    ...
    if (!Advancement)
      throw new Error(
        `${type}Advancement not found in dnd5e.advancement.types`
      );
    ...
    const config = new Advancement.metadata.apps.config(
      this.advancement.byId[data._id]
    );
  ...
  async deleteAdvancement(id) {
    if (!this.system.advancement) return;
    return this.update({
      "system.advancement": this.system.advancement.filter((a) => a._id !== id),
    });
  }
  ...
  async _preUpdate(changed, options, user) {
    ...
    if (changed.system.levels > CONFIG.DND5E.maxLevel) {
      ui.notifications.warn(
        game.i18n.format("DND5E.MaxClassLevelExceededWarn", {
  ...
  _onCreateOwnedEquipment(data, isNPC) {
    ...
    if (foundry.utils.getProperty(data, "system.proficient") === undefined) {
      if (isNPC) {
        ...
        const armorProf =
          CONFIG.DND5E.armorProficienciesMap[this.system.armor?.type]; // Player characters check proficiency
        const actorArmorProfs =
          this.parent.system.traits?.armorProf?.value || [];
  ...
  _onCreateOwnedTool(data, isNPC) {
    ...
    if (data.system?.proficient === undefined) {
      if (isNPC) updates["system.proficient"] = 1;
        ...
        const proficient =
          actorToolProfs.includes(this.system.toolType) ||
  ...
  _onCreateOwnedWeapon(data, isNPC) {
    ...
    const weaponProf =
      CONFIG.DND5E.weaponProficienciesMap[this.system.weaponType];
    const updates = {};
    ...
    // Characters may have proficiency in this weapon type (or specific base weapon)
      ...
      updates["system.proficient"] =
        actorProfs.includes(weaponProf) ||
        actorProfs.includes(this.system.baseItem);
    }
    return updates;
  }
...
class SelectItemsPrompt extends Dialog {
  ...
  static async create(items, { hint }) {
    // Render the ability usage template
    const html = await renderTemplate(
      "systems/dnd5e/templates/apps/select-items-prompt.hbs",
      { items, hint }
    );
    return new Promise((resolve) => {
        ...
        buttons: {
          apply: {
            ...
            callback: (html) => {
              ...
              const selectedIds = Object.keys(fd).filter(
                (itemId) => fd[itemId]
              );
              resolve(selectedIds);
            },
...
class Actor5e extends Actor {
  ...
  _prepareBaseAbilities(updates) {
    ...
    for (const key of Object.keys(CONFIG.DND5E.abilities)) {
      ...
      if (!abilities[key]) {
        ...
        updates[`system.abilities.${key}`] = foundry.utils.deepClone(
          abilities[key]
        );
      }
    }
  ...
  _prepareCharacterData() {
    ...
    this.system.attributes.prof = Math.floor(
      (this.system.details.level + 7) / 4
    );
      ...
      default:
        ...
        try {
          ...
          const replaced = Roll.replaceFormulaData(
            CONFIG.DND5E.armorClasses.default.formula,
            rollData
          );
          ac.base = Roll.safeEval(replaced);
        }
  ...
  _prepareInitiative(bonusData, globalCheckBonus) {
    ...
    const { initiativeAlert, jackOfAllTrades, remarkableAthlete } =
      this.flags.dnd5e ?? {};

    // Initiative modifiers
    const dexCheckBonus = simplifyBonus(
      this.system.abilities.dex?.bonuses?.check,
      bonusData
    );
  ...
  _prepareSpellcasting() {
    ...
    if (
      !isNPC &&
      isSingleClass &&
      ["half", "third"].includes(caster.spellcasting.progression)
    ) {
    ...
    if (pl === 0 && isNPC && Number.isNumeric(spells.pact.override))
      pl = this.system.details.spellLevel;
  ...
  _isRemarkableAthlete(ability) {
    return (
      this.getFlag("dnd5e", "remarkableAthlete") &&
  ...
  async rollSkill(skillId, options = {}) {
    ...
    const flavor = game.i18n.format("DND5E.SkillPromptTitle", {
      skill: CONFIG.DND5E.skills[skillId]?.label ?? "",
    });
     ...
     */
    if (Hooks.call("dnd5e.preRollSkill", this, rollData, skillId) === false)
      return;
  ...
  rollAbility(abilityId, options = {}) {
    ...
    new Dialog({
      title: `${game.i18n.format("DND5E.AbilityPromptTitle", {
        ability: label,
      })}: ${this.name}`,
      content: `<p>${game.i18n.format("DND5E.AbilityPromptText", {
        ability: label,
      })}</p>`,
      buttons: {
        test: {
  ...
  async rollAbilityTest(abilityId, options = {}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("DND5E.AbilityPromptTitle", {
      ability: label,
    });
    const rollData = foundry.utils.mergeObject(
      {
     ...
     */
    if (
      Hooks.call("dnd5e.preRollAbilityTest", this, rollData, abilityId) ===
      false
    )
      return;
  ...
  async rollAbilitySave(abilityId, options = {}) {
    ...
    // Roll and return
    const flavor = game.i18n.format("DND5E.SavePromptTitle", {
      ability: label,
    });
    const rollData = foundry.utils.mergeObject(
      {
     ...
     */
    if (
      Hooks.call("dnd5e.preRollAbilitySave", this, rollData, abilityId) ===
      false
    )
      return;
  ...
  async rollDeathSave(options = {}) {
     ...
     */
    if (Hooks.call("dnd5e.rollDeathSave", this, roll, details) === false)
      return roll;
  ...
  async rollHitDie(denomination, options = {}) {
     ...
     */
    if (
      Hooks.call("dnd5e.preRollHitDie", this, rollConfig, denomination) ===
      false
    )
      return;
     ...
     * @returns {boolean}             Explicitly return `false` to prevent updates from being performed.
     */
    if (Hooks.call("dnd5e.rollHitDie", this, roll, updates) === false)
      return roll;
  ...
  async rollClassHitPoints(item) {
    ...
    const flavor = game.i18n.format("DND5E.AdvancementHitPointsRollMessage", {
      class: item.name,
    });
    const messageData = {
      title: `${flavor}: ${this.name}`,
  ...
  async _displayRestResultMessage(result, longRest = false) {
    ...
    switch (game.settings.get("dnd5e", "restVariant")) {
      case "normal":
        restFlavor =
          longRest && newDay
            ? "DND5E.LongRestOvernight"
            : `DND5E.${length}RestNormal`;
        break;
      case "gritty":
        restFlavor =
          !longRest && newDay
            ? "DND5E.ShortRestOvernight"
            : `DND5E.${length}RestGritty`;
        break;
      case "epic":
    ...
    if (diceRestored && healthRestored) message = `DND5E.${length}RestResult`;
      ...
      message = "DND5E.LongRestResultHitPoints";
    else if (longRest && diceRestored && !healthRestored)
      message = "DND5E.LongRestResultHitDice";
    ...
    for (let item of this.items) {
      ...
      if (recoverDailyUses && uses?.recovery && uses?.per === "charges") {
        ...
        try {
          ...
          ui.notifications.warn(
            game.i18n.format("DND5E.ItemRecoveryFormulaWarning", {
              name: item.name,
        ...
        if (newValue !== uses.value) {
          ...
          const locKey = `DND5E.Item${diff < 0 ? "Loss" : "Recovery"}Roll${
            isMax ? "Max" : ""
          }`;
  ...
  async transformInto(
    ...
    // Set new data flags
    if (!this.isPolymorphed || !d.flags.dnd5e.originalActor)
      d.flags.dnd5e.originalActor = this.id;
  ...
  async revertOriginalForm() {
    ...
    if (!this.isOwner) {
      return ui.notifications.warn(
        game.i18n.localize("DND5E.PolymorphRevertWarn")
      );
    }
  ...
  static prepareProficiencies(data, type) {
    ...
    for (const key of values) {
      if (profs[key]) {
        ...
        const item = ProficiencySelector.getBaseItem(itemTypes[key], {
          indexOnly: true,
        });
  ...
  async addEmbeddedItems(items, prompt = true) {
    foundry.utils.logCompatibilityWarning(
      "Actor5e#addEmbeddedItems has been deprecated.",
      { since: "DnD5e 1.6", until: "DnD5e 2.1" }
    );
...
class ActorAbilityConfig extends DocumentSheet {
  ...
  get title() {
    return `${game.i18n.format("DND5E.AbilityConfigureTitle", {
      ability: CONFIG.DND5E.abilities[this._abilityId],
    })}: ${this.document.name}`;
  }
  ...
  getData(options) {
    ...
    return {
      ...
      labelSaves: game.i18n.format("DND5E.AbilitySaveConfigure", {
        ability: CONFIG.DND5E.abilities[this._abilityId],
      }),
      labelChecks: game.i18n.format("DND5E.AbilityCheckConfigure", {
        ability: CONFIG.DND5E.abilities[this._abilityId],
      }),
  ...
  _getBonuses() {
    ...
    const bonuses = [
      ...
      { name: "system.bonuses.rsak.damage", label: "DND5E.BonusRSDamage" },
      {
        ...
        label: "DND5E.BonusAbilityCheck",
      },
      {
        name: "system.bonuses.abilities.save",
        label: "DND5E.BonusAbilitySave",
      },
      {
        name: "system.bonuses.abilities.skill",
        label: "DND5E.BonusAbilitySkill",
      },
  ...
  async _updateObject(event, formData) {
    ...
    for (let [k, v] of Object.entries(flags)) {
      if ([undefined, null, "", false, 0].includes(v)) {
        delete flags[k];
        if (foundry.utils.hasProperty(src.flags, `dnd5e.${k}`))
          flags[`-=${k}`] = null;
      }
    }
...
class ActorSkillConfig extends DocumentSheet {
  ...
  get title() {
    ...
    return `${game.i18n.format("DND5E.SkillConfigureTitle", {
      skill: label,
    })}: ${this.document.name}`;
  }
...
class ActorTypeConfig extends FormApplication {
  ...
  /** @inheritDoc */
  get title() {
    return `${game.i18n.localize("DND5E.CreatureTypeTitle")}: ${
      this.object.name
    }`;
  }
  ...
  getData(options = {}) {
    ...
    return {
      ...
      swarm: attr.swarm,
      sizes: Array.from(Object.entries(CONFIG.DND5E.actorSizes))
        .reverse()
        .reduce((obj, e) => {
          obj[e[0]] = e[1];
          return obj;
...
class ActorSheet5e extends ActorSheet {
      ...
      default:
        ...
        if (armorInFormula)
          label =
            ...
            game.i18n.localize("DND5E.ArmorClassUnarmored");
        attribution.unshift({
          label,
          mode: CONST.ACTIVE_EFFECT_MODES.OVERRIDE,
          value: base,
    ...
    if (ac.shield !== 0)
      attribution.push({
        label:
          ...
          game.i18n.localize("DND5E.EquipmentShield"),
        mode: CONST.ACTIVE_EFFECT_MODES.ADD,
        value: ac.shield,
      });
  ...
  _prepareTraits(traits) {
    ...
    for (const [key, choices] of Object.entries(map)) {
      ...
      if (!trait) continue;
      let values =
        (trait.value ?? []) instanceof Array ? trait.value : [trait.value];
      ...
      if (physical.length) {
        ...
        trait.selected.physical = game.i18n.format(
          "DND5E.DamagePhysicalBypasses",
          {
            ...
            bypassTypes: bypassFormatter.format(
              trait.bypasses.map((t) => config.physicalWeaponProperties[t])
            ),
          }
        );
      }
  ...
  activateListeners(html) {
    ...
    if (this.isEditable) {
      ...
      // Trait Selector
      html
        .find(".proficiency-selector")
        .click(this._onProficiencySelector.bind(this));
  ...
  async _onPropertyAttribution(event) {
    ...
    const html = await new PropertyAttribution(
      this.actor,
      attributions,
      property
    ).renderTooltip();
  ...
  _onProficiencySelector(event) {
    ...
    const options = {
      ...
      title: `${label.innerText}: ${this.actor.name}`,
      type: a.dataset.type,
    };
...
class ActorSheet5eCharacter extends ActorSheet5e {
  ...
  async getData(options = {}) {
    ...
    return foundry.utils.mergeObject(context, {
      disableExperience: game.settings.get(
        "dnd5e",
  ...
  _prepareItems(context) {
    ...
    classes = classes.reduce((arr, cls) => {
      cls.availableLevels = Array.fromRange(CONFIG.DND5E.maxLevel + 1)
        .slice(1)
        .map((level) => {
    ...
    const features = {
      background: {
        label: "DND5E.ItemTypeBackground",
        items: backgrounds,
        hasActions: false,
  ...
  async _onDropSingleItem(itemData) {
    ...
    // If a subclass is dropped, ensure it doesn't match another subclass with the same identifier
      ...
      if (other) {
        const err = game.i18n.format("DND5E.SubclassDuplicateError", {
      ...
      if (cls && cls.subclass) {
        const err = game.i18n.format("DND5E.SubclassAssignmentError", {
          class: cls.name,
...
class ActorSheet5eNPC extends ActorSheet5e {
  ...
  getArmorLabel() {
    ...
    if (ac.calc === "default")
      ...
      label.push(game.i18n.localize(CONFIG.DND5E.armorClasses[ac.calc].label));
...
class ActorSheet5eVehicle extends ActorSheet5e {
  ...
  _prepareItems(context) {
    ...
    context.system.attributes.encumbrance = this._computeEncumbrance(
      totalWeight,
      context
    );
  }
...
class ItemSheet5e extends ItemSheet {
  ...
  _onAdvancementAction(target, action) {
    ...
    switch (action) {
      case "add":
        return game.dnd5e.advancement.AdvancementSelection.createDialog(
          this.item
        );
      case "edit":
      ...
      case "modify-choices":
        ...
        const manager = AdvancementManager.forModifyChoices(
          this.item.actor,
          this.item.id,
          Number(level)
        );
...
class SRDCompendium extends Compendium {
  ...
  async getData(options) {
    ...
    const getOrder = (o) =>
      (({ chapter: 0, appendix: 100 }[o.flags?.dnd5e?.type] ?? 200) +
      (o.flags?.dnd5e?.position ?? 0));
  ...
  configureDamage() {
    ...
    if (this.options.powerfulCritical && flatBonus > 0) {
      ...
      this.terms.push(
        new NumericTerm(
          { number: flatBonus },
          { flavor: game.i18n.localize("DND5E.PowerfulCritical") }
        )
      );
    }
...
function _migrateActorMovement(actorData, updateData) {
  ...
  if (hasOld) {
    ...
    if (!hasNew && typeof old === "string") {
      const s = (old || "").split(" ");
      if (s.length > 0)
        updateData["system.attributes.movement.walk"] = Number.isNumeric(s[0])
          ? parseInt(s[0])
          : null;
    }
...
function _migrateActorSenses(actor, updateData) {
  ...
  for (let s of oldSenses.split(",")) {
    ...
    if (type in CONFIG.DND5E.senses) {
      updateData[`system.attributes.senses.${type}`] = Number(
        match[2]
      ).toNearest(0.5);
      wasMatched = true;
    }
...
function _migrateActorType(actor, updateData) {
  ...
  if (match) {
    ...
    const typeMatch = Object.entries(CONFIG.DND5E.creatureTypes).find(
      ([k, v]) => {
        return (
          typeLc === k ||
    ...
    // Match a swarm
    const isNamedSwarm = actor.name?.startsWith(
      game.i18n.localize("DND5E.CreatureSwarm")
    );
    if (match.groups.size || isNamedSwarm) {
      const sizeLc = match.groups.size
        ? match.groups.size.trim().toLowerCase()
        : "tiny";
      const sizeMatch = Object.entries(CONFIG.DND5E.actorSizes).find(
        ([k, v]) => {
...
function _migrateItemRarity(item, updateData) {
  ...
  const rarity = Object.keys(CONFIG.DND5E.itemRarity).find(
    (key) =>
      CONFIG.DND5E.itemRarity[key].toLowerCase() ===
        item.system.rarity.toLowerCase() || key === item.system.rarity
  );
...
function _migrateItemSpellcasting(item, updateData) {
  if (
    item.type !== "class" ||
    foundry.utils.getType(item.system.spellcasting) === "Object"
  )
    return updateData;
...
function _migrateArmorType(item, updateData) {
  ...
  if (item.system?.armor?.type === "bonus")
    updateData["system.armor.type"] = "trinket";
  return updateData;
}
...
function _migrateItemCriticalData(item, updateData) {
  const hasCritData =
    game.system.template.Item[item.type]?.templates?.includes("action");
  if (!hasCritData || foundry.utils.getType(item.system.critical) === "Object")
    return updateData;
...
Hooks.once("init", function () {
  ...
  console.log(
    `DnD5e | Initializing the DnD5e Game System - Version ${dnd5e.version}\n${DND5E.ASCII}`
  );
...
Hooks.once("setup", function () {
  CONFIG.DND5E.trackableAttributes = expandAttributeList(
    CONFIG.DND5E.trackableAttributes
  );
  CONFIG.DND5E.consumableResources = expandAttributeList(
...
Hooks.once("ready", function () {
  ...
  const cv =
    game.settings.get("dnd5e", "systemMigrationVersion") ||
