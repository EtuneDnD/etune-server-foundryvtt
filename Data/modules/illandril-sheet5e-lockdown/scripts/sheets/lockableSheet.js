...
export const isLockableSheet = (sheetName) =>
  LOCKABLE_SHEET_NAMES.has(sheetName);
...
export default class LockableSheet {
  ...
  initialize(sheetElem, actor, isSheetEditable) {
    ...
    if (
      isSheetEditable &&
      game.user.hasRole(Settings.ShowToggleEditRole.get())
    ) {
      ...
      editOnLink.addEventListener(
        "click",
        () => this.makeLocked(sheetElem, actor, false, isSheetEditable),
        false
      );
      ...
      editOffLink.addEventListener(
        "click",
        () => this.makeLocked(sheetElem, actor, true, isSheetEditable),
        false
      );
      ...
      if (labelKeyType !== null) {
        const toggleOnString = game.i18n.localize(
          `${MODULE_KEY}.toggleEditOn${labelKeyType}`
        );
        const toggleOffString = game.i18n.localize(
          `${MODULE_KEY}.toggleEditOff${labelKeyType}`
        );
  ...
  makeLocked(sheetElem, actor, locked, isSheetEditable) {
    ...
    const hideEmptySpellbook =
      locked &&
      (Settings.HideAddItemButtons.get() || Settings.HideEmptySpellbook.get());
    const isSpellbookEmptyAndHidden =
      hideEmptySpellbook && this.isSpellbookEmpty(actor);
    lockUnlock(
    ...
    // Effects
    lockUnlock(
      this.getEffectControls(sheetElem),
      locked,
      Settings.LockEffects,
      isSheetEditable
    );
    const hideEmptyEffects = locked && Settings.LockEffects.get();
    const isEffectsEmptyAndHidden =
      hideEmptyEffects && this.isEffectsEmpty(actor);
    lockUnlock(
    ...
    // Unsorted stuff
    lockUnlock(
      this.getUnsorteds(sheetElem),
      locked,
      Settings.LockUnsorteds,
      isSheetEditable
    );
  ...
  getTraits(sheetElem) {
    return [
      ...
      Settings.ShowSpecialTraits.get()
        ? null
  ...
  getSensesInput(sheetElem) {
    return {
      elements: sheetElem.querySelectorAll(
        'input[name="system.traits.senses"]'
      ),
      lockMode: LockMode.FORM_DISABLED,
    };
  }
  ...
  getAvailableSpellSlots(sheetElem) {
    return {
      elements: sheetElem.querySelectorAll(
        'input[name^="system.spells."][name$=".value"]'
      ),
      lockMode: LockMode.FORM_DISABLED,
    };
  }
  ...
  addTag(traitsList, labelLocaleKey, value) {
    ...
    tagElem.appendChild(
      document.createTextNode(game.i18n.localize(labelLocaleKey))
    );
  ...
  getSpecialTraitsRow(sheetElem) {
    const flagsToggle = sheetElem.querySelector(
      '.traits [data-action="flags"]'
    );
    if (!flagsToggle) {
      return null;
    }
...
export const lockUnlock = (
  ...
  if (typeof isSheetEditable !== "boolean") {
    log.error(
      `isSheetEditable was not a boolean - please submit a bug report in GitHub with a screenshot of the stack trace for this error`
    );
...
export const isHideReserveSpace = (element) =>
  element.classList.contains(CSS_HIDE_RESERVE_SPACE);
...
const stopPropagation = (event) => {
  ...
  Hooks.on("renderActorSheet", (actorSheet, html, actor) => {
    setTimeout(() => {
      console.log(actor.system.details.xp);
      console.log(html[0].querySelectorAll('button[title="Level Up"]'));
      if (actor.system.details.xp.value >= actor.system.details.xp.max) {
        html[0].querySelectorAll('button[title="Level Up"]')[0].classList.remove("etune-level-up")
      } else {
        html[0].querySelectorAll('button[title="Level Up"]')[0].classList.add("etune-level-up")
      }

      const owned = game.actors.filter(actor => {
        return actor.ownership[game.userId] === CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER;
      });
